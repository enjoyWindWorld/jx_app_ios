...
- (void)setCityName:(NSString *)cityName {
    ...
    self.currentCityLabel.text = m.cityName;
}
...
- (void)addLabels {
    ...
    _currentCityLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15];
...
- (void)addJFButton {
    self.button = [[JFButton alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 34, SCREEN_WIDTH-20, 27)];
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar {
    ...
    for(UIView *view in  [[[searchBar subviews] objectAtIndex:0] subviews]) {
        ...
        if([view isKindOfClass:[NSClassFromString(@"UINavigationButton") class]]) {
            ...
            [cancel  setTintColor:SPNavBarColor];
            [cancel setFont:[UIFont systemFontOfSize:15]];
            [cancel.titleLabel setTextColor:[UIColor grayColor]];
        }
    }
