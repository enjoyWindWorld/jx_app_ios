...
- (void)viewDidLoad {
    ...
    [leftButton setImage:[UIImage imageNamed:@"cancel@3x"] forState:UIControlStateNormal];
...
- (void)chooseCityWithName:(NSNotification *)info {
    ...
    if ([[cityDic valueForKey:@"SubLocality"] isEqualToString:@"全城"]) {
        ...
        if (cityname.length == 0) {
            ...
            cityName = [cityDic valueForKey:@"SubLocality"];
...
- (JFCityHeaderView *)headerView {
    if (!_headerView) {
        ...
        _headerView.layer.borderWidth = 0.7;
        _headerView.layer.borderColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:215/255.0 alpha:1.0].CGColor;
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        [_searchView resultBlock:^(NSDictionary *cityData) {
            [KCURRENTCITYINFODEFAULTS setObject:[cityData valueForKey:@"super"] forKey:@"currentCity"];
            [KCURRENTCITYINFODEFAULTS setObject:[cityData valueForKey:@"city"] forKey:@"searchCity"];
            [KCURRENTCITYINFODEFAULTS setObject:[cityData valueForKey:@"city_number"] forKey:@"cityNumber"];
            DELog(@"cityname is %@ %@",[cityData valueForKey:@"super"],[cityData valueForKey:@"city"]);
            if (weakSelf.choseCityBlock) {
                //顶层城市
                NSString *superCity = [cityData valueForKey:@"super"];
                if (superCity.length > 0) {
                    //当搜索是城市和区共有的时候
                    model.city = [cityData valueForKey:@"super"];
                    model.SubLocality = [cityData valueForKey:@"city"];
                    DELog(@"sublocality is %@ %@",model.city,model.SubLocality);
                    ...
                    //当只搜到城市的时候
                    model.city = [cityData valueForKey:@"city"];
                    model.SubLocality = nil;
                    [model saveUserLoginModel];
...
- (NSMutableArray *)characterMutableArray {
    if (!_characterMutableArray) {
        _characterMutableArray = [NSMutableArray arrayWithObjects:UITableViewIndexSearch,@"定位", @"最近", @"热门", nil];
    }
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        ...
        [_rootTableView setSectionIndexColor:SPNavBarColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < _HeaderSectionTotal) {
        ...
        if (indexPath.section == _HeaderSectionTotal - 3) {
            ...
            //_cell.cityNameArray = @[self.locationCity];
        }
...
{
    //修改标题属性
    view.tintColor = JFRGBColor(247, 247, 247);
    UITableViewHeaderFooterView *header = (UITableViewHeaderFooterView *)view;
    header.contentView.backgroundColor=[UIColor clearColor];
    header.textLabel.textAlignment=NSTextAlignmentLeft;
    [header.textLabel setTextColor:[UIColor grayColor]];
    header.textLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15];
...
{
    if (_HeaderSectionTotal == 3) {
        if (indexPath.section == 1||indexPath.section == 2||indexPath.section == 0){
            cell.backgroundColor = JFRGBColor(247, 247, 247);
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [_manager cityNumberWithCity:cell.textLabel.text cityNumber:^(NSString *cityNumber) {
        ...
        if (model) {
            model.city = cell.textLabel.text;
            model.SubLocality = nil;
            [model saveUserLoginModel];
        }
    }];
