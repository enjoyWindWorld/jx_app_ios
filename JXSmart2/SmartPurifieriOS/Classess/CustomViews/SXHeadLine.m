...
-(void)setIsOpenTouch:(BOOL)isOpenTouch{


    _isOpenTouch = isOpenTouch;
...
- (void)scrollAnimate
{
    ...
    NSInteger index = (self.messageIndex + self.messageArray.count - 2)%self.messageArray.count;
    
    if (self.animateBlock) {
        
        self.animateBlock(index);
    }
...
- (void)checkLabelFrameChange:(UILabel *)label
{
    if (label.frame.origin.y < -10) {
        ...
        if (self.lastBlock&&self.messageIndex==2) {
