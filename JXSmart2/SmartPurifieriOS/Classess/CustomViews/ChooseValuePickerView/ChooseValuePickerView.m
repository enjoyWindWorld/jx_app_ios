...
@interface ChooseValuePickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation ChooseValuePickerView
...
- (void)initSubViews
{
    ...
    [self.finishBtn setTitleColor:[UIColor colorWithHexString:@"777777"] forState:UIControlStateNormal];
    ...
    [self.cancelBtn setTitleColor:[UIColor colorWithHexString:@"777777"] forState:UIControlStateNormal];
...
- (void)layoutSelfSubviews
{
    ...
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, SCREEN_WIDTH, 42);
...
-(void)setDefaultChoose:(NSInteger)defaultChoose{

    _defaultChoose = defaultChoose ;
    
     [self.statePicker selectRow:defaultChoose inComponent:0 animated:NO];
...
- (void)finishBtnClicked:(UIButton *)button
{
    if (self.valueDidSelect) {
        ...
        self.valueDidSelect(_defaultChoose);
    }
