...
+ (BOOL)qsh_ReadCacheforParam:(id)param url:(NSString*)url successBlock:(RequestSuccess)success{
    ...
    NSString * key = [NSString stringWithFormat:@"%@/%@%@",SmartPurifierHostURL,url,param==nil?@"{}":[param mj_JSONString]];
...
+(NSArray*)arrContainsUrl{
    ...
    return @[homeListFile,HomePageData,pushtotalURL,doultonDetailsURL,serviceListURL,cleanDeitalURL,productDetailDataURL,communityGetAdvURL,serviceListURL,userMerchantRelease,userMyOrder,userMessageList,HomePageNewsList,MovieHappylyList,fetch_chooseProductlist];
}
