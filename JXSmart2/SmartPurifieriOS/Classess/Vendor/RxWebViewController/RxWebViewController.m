...
-(UIWebView*)webView{
    if (!_webView) {
        ...
        rect.size.height-=44+[UIApplication sharedApplication].statusBarFrame.size.height;
...
@interface SPAgreetmentWebViewController ()
...
@implementation SPAgreetmentWebViewController
...
-(void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    [self.view bringSubviewToFront:self.confirmBT];
    
}

-(void)confirmAgreetment{
...
-(UIButton *)confirmBT{
    ...
    if (_confirmBT == nil) {
       ...
       _confirmBT = [UIButton buttonWithType:UIButtonTypeCustom];
        ...
        [_confirmBT setTitle:@"同意服务协议" forState:UIControlStateNormal];
        
        [_confirmBT setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        
        [_confirmBT setBackgroundColor:SPNavBarColor];
        
        _confirmBT.layer.masksToBounds = YES ;
        
        [_confirmBT addTarget:self action:@selector(confirmAgreetment) forControlEvents:UIControlEventTouchUpInside];
        
        _confirmBT.layer.cornerRadius = 4.0;
        
        _confirmBT.frame = CGRectMake(15, SCREEN_HEIGHT-115, SCREEN_WIDTH-30, 44);
