...
- (void)viewDidLoad {
    ...
    UIBarButtonItem * btn_Item = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"nav_back_left"] style:UIBarButtonItemStylePlain target:self action:@selector(backrootTableViewController)];
...
//        _areaMutableArray = [NSMutableArray arrayWithObject:@"全城"];
        _areaMutableArray = [NSMutableArray arrayWithCapacity:0];
    }
...
//            }];
           ...
           __block NSMutableArray * searchArr = [NSMutableArray arrayWithCapacity:0];
            
            [weakSelf.cityMutableArray enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL * _Nonnull stop) {
               
                if ([obj containsString:result]) {
                    
                    NSDictionary *dataDic = @{@"super":@"",@"city":obj,@"city_number":@""};
                    
                    [searchArr addObject:dataDic];
                }
                
            }];
            if (searchArr.count == 0) {
                [searchArr addObject:@{@"city":@"抱歉",@"super":@"未找到相关位置，可尝试修改后重试!"}];
            }
            
            _searchView.backgroundColor = [UIColor whiteColor];
            _searchView.resultMutableArray = searchArr.mutableCopy;
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        [_searchView resultBlock:^(NSDictionary *cityData) {
            ...
            NSString * city = [cityData valueForKey:@"city"] ;
...
- (NSArray *)hotCityArray {
    if (!_hotCityArray) {
        _hotCityArray = @[@"广州市", @"深圳市", @"武汉市", @"西安市", @"南京市", @"杭州市",@"成都市",@"汕头市",@"合肥市",@"沈阳市"];
    }
...
- (void)scrollToTop {
    ...
    [self.rootTableView setContentOffset:CGPointMake(0,0) animated:YES];
}
