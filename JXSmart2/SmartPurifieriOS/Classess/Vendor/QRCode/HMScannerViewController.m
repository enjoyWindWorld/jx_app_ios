...
- (void)viewDidLoad {
    ...
    scanner = [HMScanner scanerWithView:self.view scanFrame:scannerBorder.frame completion:^(NSString *stringValue) {
        ...
        if ([stringValue rangeOfString:@"jxsmart"].location != NSNotFound) {
            
            if ([stringValue rangeOfString:@"promotion"].location != NSNotFound) {
                
                if (stringValue.length>16) {
                    
                    NSString * encrystr = [SPSmartInterfaceEncryption decryptionResponseData:[stringValue substringFromIndex:INTERPROMOTION.length] url:nil];
                    // 完成回调
                    weakSelf.completionCallBack(encrystr);
                    // 关闭
                    [weakSelf clickCloseButton];
                }
            }else{

                [SPToastHUD makeToast:@"对不起，扫描的不是净喜的推广码" duration:3 position:nil makeView:weakSelf.view];
            }
        }else{

            [SPToastHUD makeToast:@"对不起，扫描的不是净喜的二维码" duration:3 position:nil makeView:weakSelf.view];
        }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    [HMScanner scaneImage:image completion:^(NSArray *values) {
        ...
        if (values.count > 0) {
            ...
            NSString * stringValue = [values firstObject];
...
- (void)prepareOtherControls {
    ...
    [cardButton setTitle:@"我的推广码" forState:UIControlStateNormal];
