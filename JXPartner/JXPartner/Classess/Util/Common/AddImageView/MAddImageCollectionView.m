    ...
    if (images.count==0 ) {
        ...
        if (images.count<self.maxImageCount && indexPath.row == images.count) {
            ...
            if ([object isKindOfClass:[ALAsset class]]) {
                ...
                [cell.iconImage imageWithURL:model.icon placeholderImage:SPPRODUCTICOPLACEHOLDERImage];
            }else if ([object isKindOfClass:[NSString class]]){

                [cell.iconImage imageWithURL:object placeholderImage:SPPRODUCTICOPLACEHOLDERImage];
            }
        }
        return cell;
    }
                                     ...
                                     completionBlock:^(NSURL *assetURL, NSError *error)
     {
         if (!error) {
             [self.assetsLibrary assetForURL:assetURL resultBlock:^(ALAsset *asset) {
                 ...
                 [self makeToast:@"选择图片失败,请重新选择" duration:3 position:nil];
             }];
         }else
         {
              [self makeToast:@"选择图片失败,请重新选择" duration:3 position:nil];
